...
local function make_wangzha(real_card_num_set,card_type_list)
    --print("00000000000000000",tostring_r(real_card_num_set))
...
local function select_card_type_list(uid,ddz_instance,tmp_card_type_list)
    ...
    local tmp_absolute_count = 0
    if tmp_handle_count <= 4 then
        tmp_absolute_count = get_absolute_handle_count(uid,ddz_instance,tmp_card_type_list)
    end

    local tmp_weigh_value = get_card_weigh_value(tmp_card_type_list)
    if tmp_absolute_count >= tmp_handle_count - 1 then --满足必赢条件
    ...
    local cur_absolute_count = 0
    if cur_handle_count <= 4 then
        cur_absolute_count = get_absolute_handle_count(uid,ddz_instance,make_card_tmp_list)
    end 
    ...
    print("select_card_type_list9999999",get_now_ustime() - start_time)
end
...
local function make_from_disorder_card(real_card_number_set,tmp_card_type_list,disorder_card_map)
    ...
    print("make_from_disorder_card1111",get_now_ustime() - start_time)
end
...
make_card_type = function(uid,ddz_instance)
    ...
    if #zd_sz_dz_combine_list <= 2 then
    ...
    local elapsed = get_now_ustime() - start_time
    print("make_card_type =================end===================================================",elapsed,uid,#zd_sz_dz_combine_list)
    --print("card +++++++++++++++++++++++++",tostring_r(zd_sz_dz_combine_list),tostring_r(tmp_real_card_number_set))
    print("ttttttttttttttttttttttttttttttt",tostring_r(cards_id_list),tostring_r(make_card_tmp_list))
...
local function check_dan_pai(self,last_card_suit_key,only_check_dan_pai)
    ...
    if last_card_suit_key == CARD_TWO_NUMBER then
        if real_card_number_set[BLACK_JOKER_NUMBER] then --小王压2
    ...
    for _,number in pairs(dan_pai_list) do
        if POWER_MAP[last_card_suit_key] < POWER_MAP[number] then
            if number == RED_JOKER_NUMBER and rival_can_must_win(self) then
               return {[RED_JOKER_NUMBER] = 1}
            end
            if number == BLACK_JOKER_NUMBER then
                break
            end
            return {[number] = 1}
        end
...
local function get_feiji(last_card_suit_key,last_card_suit,feiji_len)
    --拆相同张数的飞机
    for _,feiji in pairs(self_card_type_list[CARD_SUIT_TYPE_FEIJI]) do
        local max_num = feiji[#feiji]
...
local function check_feiji(last_card_suit_key,last_card_suit)
    ...
    local ret = get_feiji(last_card_suit_key,last_card_suit,#sanzhang_list)
    if ret then
        return ret
    end
...
local function check_feiji_and_wing(last_card_suit_key,last_card_suit)
    ...
    --拆相同张数的飞机
    local ret = get_feiji(last_card_suit_key,last_card_suit,feiji_len)
    if ret then
...
local function rival_is_remain_dui_pai(self)
    ...
    if self.uid == self.ddz_instance:get_dizhu_uid() then
        ...
        local next_position_uid = self.ddz_instance:get_next_position_uid(self.uid)
        local cards_id_list = self.ddz_instance:get_player_card_ids(next_position_uid)
...
local function check_next_rival_remain_report(self,last_record)
    ...
    if last_record.card_suit_type == CARD_SUIT_TYPE_DUIPAI and
       rival_is_remain_dui_pai(self) then
       --敌方是对牌的时候出对牌
       ret = get_max_cards_greater_than_key(last_record)
    end
    if ret then 
