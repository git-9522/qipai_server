...
local function get_absolute_handle_count(uid,ddz_instance,total_card_type_list)
   ...
   local all_cards = ddz_instance:get_all_cards()
   local record_list = ddz_instance:get_card_record_list()
   local except_map = {}
   for _,card_record in pairs(record_list) do
       for _,card_id in pairs(card_record.card_suit) do
           record_map[card_id] = true
       end
   end
   local self_cards = ddz_instance:get_player_card_ids(uid)
   for _,card_id in pairs(self_cards) do
       except_map[card_id] = true
   end
   local other_player_cards = {}
   for _,card_id in pairs(all_cards) do
       if not except_map[card_id] then
           table_insert(other_player_cards,card_id)
       end
   end
   ...
   for card_type,card_type_list in pairs(total_card_type_list) do
       for _,card_number in pairs(card_type_list) do
           ...
           if ret then
              ...
              if not next(lz_remind.card_remind(other_player_cards,card_suit,card_type,key))then
