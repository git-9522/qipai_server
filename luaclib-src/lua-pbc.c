...
static struct pbc_env * G_pbc_env = NULL;
static int
_env_init_single(lua_State *L){
	if(G_pbc_env == NULL){
		G_pbc_env = pbc_new();
	}
	return 0;
...
static int
_env_get(lua_State *L) {
	if(G_pbc_env == NULL){
		return 0;
	}
	lua_pushlightuserdata(L, G_pbc_env);
	return 1;
}
...
luaopen_protobuf_c(lua_State *L) {
	luaL_Reg reg[] = {
		{"_env_init_single" , _env_init_single },
		{"_env_get" , _env_get },
		{"_env_new" , _env_new },
