...
function M.start(delegate)
	...
	function proxy_handler.REGISTERED(fd,ok)
		...
		if delegate.on_registered then
			delegate.on_registered(fd)
		end
	end
	...
	local function dispatch_msg(fd, msg, sz)
		local params = {skynet.unpack(msg,sz)}
		skynet.trash(msg,sz)    --free
		local cmd = params[1]
		proxy_handler[cmd](fd,select(2,table.unpack(params)))
	end
	...
	function CMD.start(conf)
		server_id = assert(conf.server_id)
		server_name = assert(conf.server_name)
		if delegate._start then
			delegate._start(conf)	
		end
